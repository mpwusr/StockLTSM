...
4. In the GUI:
   ...
   * Choose a data source— there are three sources - (Polygon, Yahoo, AlphaVantage)
   * Please focus on Yahoo as the included .env file does not have API keys for
   * Polygon or Alphavantage so Yahoo is only option without required API keys
   * leave included .env file as is with blank ="" this will default to Yahoo
